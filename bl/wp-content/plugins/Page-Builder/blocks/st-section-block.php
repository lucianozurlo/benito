...
class ST_Row_Block extends AQ_Block {
	...
	function after_form($instance) {
		...
		?>
		<div style="background-color: #e1e1e1; padding: 10px 5px;">
			...
			<div class="description half ">
                ...
                Background color <br/><em style="font-size: 0.8em;">(Chosen background color)</em><br/>
                <?php echo aq_field_color_picker('color', $block_id, $color, $default = '#f6f6f6') ?>
                </label>
            </div>
			<div class="tab-desc description" style="clear: both;">
				<label for="<?php echo $this->get_field_id('image') ?>">
					<b>Background Image  </b><em style="font-size: 0.8em;">(Recommended size: 2560 x 1440 pixel)</em>
	...
	/**** Block Outer Ouput ****/
 	...
 	function before_block($instance) {
		...
		if($image!=''){ $bg_style = 'style="background-image:url('.esc_url($image).')"'; }else{
			$bg_style = '';
		}
		$style = '';
		if($image!='' or $color!=''){ 
		...
		if(strpos($class,'parallax')){
			$parallax = 'data-stellar-background-ratio="0"';
			$clear = '';
		}else{
			$parallax ='';
			$clear ='clearfix';
 		...
 		if (!empty($tagdiv)){
 			echo '<section '.$s_id.' class="'.$class.' section-content '.$clear.'" '.$parallax.'>';
			if($title!=''){ ?>
				<div class="title-section">
					<div class="container triggerAnimation animated" data-animate="bounceIn">
						<h1><?php echo do_shortcode(htmlspecialchars_decode($title)); ?></h1>
						<p><?php echo $sub_title; ?></p>
					</div>
				</div>
			<?php }
			echo '<'.$tagdiv.' '.$style.' class="'.$class1.'" '.$d_id.' data-stellar-background-ratio="0">';
 		}else {
 			echo '<section '.$style.' '.$s_id.' class="'.$class.' section-content '.$clear.'" '.$parallax.'>';
			if($title!=''){ ?>
			<div class="title-section">
				<div class="container triggerAnimation animated" data-animate="bounceIn">
